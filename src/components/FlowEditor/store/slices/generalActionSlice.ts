...
import { uuid } from "@/components/FlowEditor/utils/uuid";
...
> = (set, get) => ({
  ...
  selectElements: (ids, expendSelection = false) => {
    get().internalUpdateSelection(
      expendSelection ? [...get().selectedKeys, ...ids] : ids,
      {
  ...
  copySelection: async () => {
    ...
    const nodes = Object.values(flattenNodes).filter((n) =>
      selectedKeys.includes(n.id)
    );
    const edges = Object.values(flattenEdges).filter((n) =>
      selectedKeys.includes(n.id)
    );

    const selectedElements: InternalClipboardData = { nodes, edges };
    const jsonBlob = new Blob([JSON.stringify(selectedElements)], {
      type: "text/html"
    });
    const textBlob = new Blob([JSON.stringify(selectedElements)], {
      type: "text/plain"
    });
  ...
  pasteIntoFlow: (data) => {
    ...
    // 最后将新的节点添加到选区
    const newSelections: string[] = Object.keys(nodes).concat(
      Object.keys(edges)
    );
