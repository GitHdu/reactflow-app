...
import { convertNodeChange } from "@/components/FlowEditor/utils/convertChange";
import {
  ActionOptions,
  ActionPayload,
  FlattenNodes,
...
> = (set, get) => ({
  ...
  updateNodeState: (id, key, value, options) => {
    get().dispatchNodes(
      { type: "updateNodeState", id, key: key as any, value },
      options
    );
  },
  ...
  updateNodeContent: (id, key, value, options) => {
    get().dispatchNodes(
      { type: "updateNodeContent", id, key: key as any, value },
      options
    );
  },
